        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setTitle(
                `\`${queue?.songs[0].name}\`를 찾을 수 없습니다.`
              ),
          ],
        })
        .then((msg) => {
      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`🎵 대기열에 더 이상 재생할 노래가 없습니다.`),
          ],
        })
      queue.stop();
    });
      ...
      message.channel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`\`${quary}\`를 찾을 수 없습니다.`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `${client.config.emoji.ERROR} \`${quary}\`의 검색결과가 없습니다.`
              ),
          ],
        })
        .then((msg) => {
        ...
        queue.textChannel
          .send({
            embeds: [
              new EmbedBuilder()
                .setColor(client.config.embed.color)
                .setDescription("음성채널에 아무도 없어 연결을 종료합니다."),
            ],
          })
          .then((msg) => {
            setTimeout(() => {
        ...
        switch (customId) {
          case "autoplay":
          {
            if (!channel) {
              return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} 먼저 음성채널에 접속하세요.**`
              );
            } else if (
              ...
              return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} 다른 음성채널에서 사용중입니다.`
              );
            } else if (!queue) {
              return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} 노래 재생중이 아닙니다.`
              );
            } else if (checkDJ) {
              return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} DJ권한이 있거나 노래 신청자만 가능합니다.`
              );
            } else if (!queue.autoplay) {
              queue.toggleAutoplay();
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 랜덤재생 활성화`
              );
            } else {
              queue.toggleAutoplay();
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 랜덤재생 비활성화`
              );
            }
          }
            break;
          case "skip":
          {
            if (!channel) {
              return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} 먼저 음성채널에 접속해주세요.`
              );
            } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
              ...
              return send(
                  interaction,
                  ` ${client.config.emoji.ERROR} 지금 바로 재생할 수 없습니다.`
              );
            } else if (checkDJ) {
              return send(
                  interaction,
              ...
              return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} 노래를 스킵했습니다.`
              );
            }
          }
            break;
          case "stop":
          {
            if (!channel) {
              ...
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 노래 재생을 멈추고 연결을 종료하였습니다.`
              );
            }
          }
            break;
          case "pauseresume":
          {
            if (!channel) {
              ...
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 노래 일시정지 해제`
              );
            } else if (!queue.paused) {
              await queue.pause();
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 노래 일시정지`
              );
            }
          }
            break;
          case "loop":
          {
            if (!channel) {
              ...
              return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} 현재 노래 반복 활성화`
              );
            } else if (queue.repeatMode === 1) {
              await queue.setRepeatMode(2);
              return send(
                  interaction,
                  `${client.config.emoji.SUCCESS} 대기열 전체 반복 활성화`
              );
            } else if (queue.repeatMode === 2) {
              await queue.setRepeatMode(0);
              return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} 반복 비활성화`
              );
            }
          }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} 음성채널에 연결 할 권한이 없습니다. **`
            );
